...
import com.mysite.mallapi.dto.MemberModifyDTO;
...
public class MemberServiceImpl implements MemberService {
	...
	private String getEmailFromKakaoAccessToken(String accessToken) {
		...
		ResponseEntity<LinkedHashMap> response = restTemplate.exchange(
				uriBuilder.toString(), HttpMethod.GET, entity,
				LinkedHashMap.class);
		log.info(response);
		...
		log.info("---------------------");
		log.info(bodyMap);
		LinkedHashMap<String, String> kakaoAccount = bodyMap
				.get("kakao_account");
	...
	private String getEmailFromGoogleAccessToken(String accessToken) {
		...
		UriComponents uriBuilder = UriComponentsBuilder
				.fromHttpUrl(googleGetUserURL).build();
		...
		return bodyMap.get("email").toString();
	...
	private String getEmailFromNaverAccessToken(String accessToken) {
		...
		UriComponents uriBuilder = UriComponentsBuilder
				.fromHttpUrl(naverGetUserURL).build();
		...
		log.info(response);
		LinkedHashMap<String, Object> bodyMap = response.getBody();
		LinkedHashMap<String, String> response2 = (LinkedHashMap<String, String>) bodyMap.get("response");
		log.info("---------------------");
		
	
		return response2.get("email");
	...
	private Member makeSocialMember(String email) {
		...
		Member member = Member.builder().email(email)
				.pw(passwordEncoder.encode(tempPassword)).nickname(nickname)
				.social(true).build();
